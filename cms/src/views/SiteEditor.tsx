...
export function SiteEditor({ email, onNavigate, onSignOut, workspace, draft }: { uid: string; email: string | null; onNavigate: (route: AppRoute) => void; onSignOut: () => void; workspace: ReturnType<typeof useDraftWorkspace>; draft: SiteDraft }) {
  ...
  const publishing = useGitHubSitePublishing({ draft, flush: workspace.flush, updateDraft: workspace.updateDraft });
  ...
  return <main className="site-editor-shell">
    ...
    {publishing.connectionOpen && <div className="draft-dialog-backdrop"><div className="draft-dialog" role="dialog" aria-modal="true"><GitFork size={23} /><h2>Connect GitHub</h2>{publishing.deviceFlow ? <><p>Enter <strong>{publishing.deviceFlow.userCode}</strong> on GitHub.</p><button className="primary-command" type="button" onClick={() => void publishing.openDevicePage()}><ExternalLink size={16} />Open GitHub</button><p>{publishing.waitingLabel}</p></> : publishing.connection?.repositoryVerified ? <><p>Ready to publish reviewed site configuration commits to <strong>{publishing.connection.repository}</strong>.</p><button className="danger-command" type="button" onClick={() => void publishing.disconnect()}>Disconnect</button></> : <><p>{publishing.connection?.message ?? 'Authorize the installed GitHub App for this repository.'}</p><button className="primary-command" type="button" onClick={() => void publishing.startConnection()}>Start connection</button></>}<div className="draft-dialog-actions"><button className="secondary-command" type="button" onClick={publishing.closeConnection}>Close</button></div></div></div>}
    ...
    {workspace.conflict && <div className="draft-dialog-backdrop"><div className="draft-dialog" role="alertdialog" aria-modal="true"><RefreshCw size={22} /><h2>This site draft changed on another device</h2><p>Your local recovery copy is intact. Use the cloud version or preserve this work as a separate draft.</p><div className="draft-dialog-actions"><button className="secondary-command" type="button" onClick={workspace.useCloudVersion}>Use cloud version</button><button className="primary-command" type="button" onClick={workspace.saveConflictAsCopy}>Save mine as copy</button></div></div></div>}
  </main>;
}
