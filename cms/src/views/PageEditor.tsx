...
export function PageEditor({
  ...
  return <main className="visual-editor-workspace page-editor-workspace">
    <header className="editor-topbar">
      ...
      <div className="draft-switcher"><label htmlFor="active-page-draft">Draft</label><div><select id="active-page-draft" value={draft.id} onChange={(event) => void workspace.selectDraft(event.target.value)}>{workspace.drafts.map((record) => <option key={record.draft.id} value={record.draft.id}>{record.draft.contentType === 'page' ? 'Page' : record.draft.contentType === 'site' ? 'Site' : 'Recipe'} · {record.draft.title || 'Untitled'}</option>)}</select><ChevronDown size={15} /></div></div>
