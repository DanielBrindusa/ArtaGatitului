...
export function PublicationHistory({ workspace }: { workspace: Workspace }) {
  ...
  return <>
    ...
    {open && <div className="draft-dialog-backdrop"><div className="history-dialog" role="dialog" aria-modal="true" aria-labelledby="history-title">
      ...
      <div className={`history-layout${!entries.length ? ' history-empty' : ''}`}>
        <aside className="history-list">
          {loading && !entries.length && <LoaderCircle className="draft-spinner" size={20} />}
          {!loading && !entries.length && <div className="history-placeholder">
            {error ? <div className="publish-error" role="alert"><CircleAlert size={16} /><span>{error}</span></div> : <p>No CMS publications yet.</p>}
            <button className="secondary-command" type="button" onClick={() => void show()}>Reload history</button>
          </div>}
          {!loading && entries.length > 0 && !visibleEntries.length && <p>No publications match this filter.</p>}
          {visibleEntries.map((entry) => <button type="button" className={details?.entry.commitSha === entry.commitSha ? 'active' : ''} key={entry.commitSha} onClick={() => void selectEntry(entry)}><span>{entry.action} · {entry.contentType}</span><strong>{entry.message}</strong><small>{entry.authoredAt ? new Date(entry.authoredAt).toLocaleString() : entry.commitSha.slice(0, 12)}</small><ChevronRight size={15} /></button>)}
        </aside>
        <section className="history-detail">
          {error && entries.length > 0 && <div className="publish-error" role="alert"><CircleAlert size={16} /><span>{error}</span></div>}
          {!details && !loading && !error && <div className="history-placeholder"><FileClock size={24} /><p>Select a CMS publication commit to inspect its files and restore supported content.</p></div>}
