...
Requesting a new code has a 35-second frontend deadline in addition to the native HTTP client's 25-second network timeout. This is not the time limit for entering a code. GitHub supplies the actual code expiration, normally 15 minutes, and the dialog displays it. A successful request displays a code, **Copy code**, and **Open GitHub**; sign in and approve in that browser, then return to the app. Final approval is deliberately performed by the user.

Polling pauses while the app is hidden behind the browser. Transport failures, temporary server errors, and rate limiting preserve the same code and retry with backoff until its real expiration. Returning to the app resumes checking; there is no need to start a new code after a temporary connection interruption. Each poll can take up to 120 seconds because a successful token exchange also verifies repository access with three additional requests. Approval is recovered from native secure storage if the frontend missed a completed response. If GitHub approval succeeded but repository verification failed, use **Check connection again** instead of generating another code. Denial, actual expiry, and configuration errors remain terminal.
