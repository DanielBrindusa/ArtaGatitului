...
Publishing runs local and native validation, loads unpublished image bytes from the current device, checks collisions against `main`, and presents an immutable review of the recipe and image paths. Confirmation rejects a moved branch, creates one Git Data API commit, and updates the branch without force. Automated tests use fixtures and issue no production write. Real OAuth, secure-storage persistence across app restart, browser return, and end-to-end publication require the manually configured GitHub App and should be tested only against an explicitly designated non-production branch. The UI intentionally distinguishes a GitHub commit from website deployment.

## Milestone 10 automated website deployment

The production content branch remains `main`; `app-development` and pull requests run validation without deploying. `.github/workflows/site.yml` uses Node.js 22, `npm ci`, and the repository's `pages:build` command to validate source, generate all public output, validate routes/indexes/links/SEO/PWA files, build the CMS, and assemble a clean `dist/site` Pages artifact. Only a successful `main` push can reach the least-privilege GitHub Pages deployment job. See `docs/deployment.md` for the audited previous branch-root deployment, workflow permissions, failure behavior, and the required one-time Pages source setting.

The CMS does not receive Actions access. After a source commit it polls the public recipe URL and verifies the exact commit marker embedded in generated HTML, distinguishing committed/building/deployed from an unconfirmed outcome without accepting an older page as success. The fixed GitHub Actions page can be opened for details; delayed and failed workflows remain deliberately indistinguishable without expanding the GitHub App permissions.

Milestone 9 verification passed the full public/CMS build, 63 Node tests, all 6 Firestore emulator tests, 12 Rust tests, strict Rust clippy, a real Windows Credential Manager write/read/delete round trip, an NSIS release build, and a hidden Windows executable startup smoke test. The ARM64 Android release library compiled with the Android Keystore-backed store, and the debug APK was assembled with the established copy fallback for Windows systems where Tauri cannot create its generated JNI symbolic link. The APK contains the expected ARM64 native library and verifies with APK Signature Scheme v2. No emulator or physical Android device is attached, so system-browser return, Android credential persistence, disconnect deletion, and publication are not claimed as device-tested. A real GitHub App Client ID and designated non-production write branch were not supplied, so no live OAuth authorization or GitHub repository write was attempted.
